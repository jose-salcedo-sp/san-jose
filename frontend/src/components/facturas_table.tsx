...
export default function FacturasTable() {
    ...
    function setSearchParams(next: Partial<{ search?: string; page?: number; perPage?: number }>) {
        navigate({
            search: (old) => {
    ...
    return <div className="space-y-4">
        <div className="flex">
            <div className="flex flex-1 items-center space-x-2">
                ...
                {(isFetching && data) && <span className="text-xs text-muted-foreground">actualizando…</span>}
            </div>
            ...
            <div className="flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                    ...
                    <Select
                        ...
                        onValueChange={(rowsPerPage) => setSearchParams({ perPage: +rowsPerPage })}
                    >
                        <SelectTrigger>
                            <SelectValue />
                        </SelectTrigger>
                ...
                <div className="flex items-center gap-2">
                    {data && <>
                        <span className="text-sm text-muted-foreground whitespace-nowrap">
                            {(page - 1) * data.perPage + 1}-{page * data.perPage} of {data.count}
                        </span>
                        <Pagination>
                            <PaginationContent>
                                <PaginationItem>
                ...
                {showingSkeleton ? (
                    ...
                    <TableRow>
                        <TableCell colSpan={9} className="text-center py-8 text-muted-foreground">
                            {rawSearch ? "No se encontraron facturas que coincidan con tu búsqueda." : "No hay facturas disponibles."}
                        </TableCell>
                    </TableRow>
                )}
            </TableBody>
